"""
Unified database diagnostics for microscopy + quant tables.
Use: python -m code.database.diagnose_db
"""
import sys
...
def print_subject_alignment(conn, cfg_subjects):
...
def print_microscopy_files(conn):
...
def print_ingest_log(conn):
    ingest_by_status = list(
        conn.execute(
            text("SELECT status, COUNT(*) AS n FROM ingest_log GROUP BY status ORDER BY status")
        )
    )
    if ingest_by_status:
...
def print_sourcedata_images():
    images_root = Path("data/sourcedata/images")
    if images_root.exists():
        pngs = list(images_root.rglob("*.png"))
        print(f"\nSourcedata images: {len(pngs)} PNGs under {images_root}")
...
def main():
    ...
    with engine.connect() as conn:
        print_counts(conn)
        print_subject_alignment(conn, cfg_subjects)
        print_microscopy_files(conn)
        print_region_counts(conn)
        print_ingest_log(conn)

    print_sourcedata_images()
